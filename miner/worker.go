...
func (w *worker) makeEnv(parent *types.Header, header *types.Header, coinbase common.Address) (*environment, error) {
	...
	state.StartPrefetcher("miner", witness)
...
func (w *worker) generateWork(params *generateParams) *newPayloadResult {
	...
	//DO OVDE
	body := types.Body{Transactions: work.txs, Withdrawals: params.withdrawals} //work.txs, 
	block, err := w.engine.FinalizeAndAssemble(w.chain, work.header, work.state, &body, work.receipts) //, params.withdrawals
	if err != nil {
		return &newPayloadResult{err: err}
	}
...
func (w *worker) commit(env *environment, interval func(), update bool, start time.Time) error {
	if w.isRunning() {
		...
		// github.com/frostymuaddib/go-ethereum-poic/issues/24299
		env := env.copy()
		// Withdrawals are set to nil here, because this is only called in PoW.
		body := types.Body{Transactions: env.txs, Withdrawals: nil} //work.txs, 
		block, err := w.engine.FinalizeAndAssemble(w.chain, env.header, env.state, &body, env.receipts) //nil
		if err != nil {
			return err
		}
